...
namespace juggle_joy.Areas.admin.Controllers
{
    public class HandlerController : Controller
    {
            ...
            if (Session["AdminID"] != null)
            {
                using (db_jugglejoyEntities ctx = new db_jugglejoyEntities())
                {
                    ...
                    if (id > 0)
                    {
                        ...
                        HandlerValidation reg = new HandlerValidation();
        ...
        public ActionResult Register(HandlerValidation obj)
        {
            if (Session["AdminID"] != null)
            {
                ModelState.Clear();
                using (db_jugglejoyEntities ctx = new db_jugglejoyEntities())
                {
                    if (ModelState.IsValid == true)
                    {
                        ...
                        else
                        {
                            ...
                            if (ctx.SaveChanges() > 0)
                            {
                                TempData["RegistrationMsg"] = "Handler added successfully";
                            }
                            else
                            {
                                TempData["RegistrationMsg"] = "Unable to add Handler";
                            }
        ...
        public JsonResult IsEmailIdExist(int UserID = 0, string EmailID = "")
        {
            using (db_jugglejoyEntities ctx = new db_jugglejoyEntities())
            {
                ...
                {
                    //code for add method
                    if (!ctx.tbl_user.Any(x => x.EmailID.ToLower() == EmailID.Trim().ToLower() && x.UserType == "H"))
                    {
                ...
                {
                    //code for edit
                    if (!ctx.tbl_user.Any(x => x.EmailID.ToLower() == EmailID.Trim().ToLower() && x.UserID != UserID && x.UserType == "H"))
                    {
                        return Json(true, JsonRequestBehavior.AllowGet);
                    }
                    else
        ...
        public JsonResult IsUserNameExixt(int UserID = 0, string UserName = "")
        {
            using (db_jugglejoyEntities ctx = new db_jugglejoyEntities())
            {
                ...
                {
                    //code for add method
                    if (!ctx.tbl_user.Any(x => x.UserName.ToLower() == UserName.Trim().ToLower() && x.UserType == "H"))
                    {
                        return Json(true, JsonRequestBehavior.AllowGet);
                    }
                    else
                ...
                {
                    //code for edit
                    if (!ctx.tbl_user.Any(x => x.UserName.ToLower() == UserName.Trim().ToLower() && x.UserID != UserID && x.UserType == "H"))
                    {
                        return Json(true, JsonRequestBehavior.AllowGet);
                    }
                    else
        ...
        public ActionResult active()
        {
            if (Session["AdminID"] != null)
            {
                using (db_jugglejoyEntities ctx = new db_jugglejoyEntities())
                {
                    ...
                    ViewBag.HandlerInUse = TempData["HandlerInUse"] == null ? "" : TempData["HandlerInUse"] as string;
                    ViewBag.HandlerNotInUse = TempData["HandlerNotInUse"] == null ? "" : TempData["HandlerNotInUse"] as string;
                    var active_assistants = ctx.tbl_user.Where(x => x.Status == (int)UserStatus.Active && x.UserType == "H").OrderByDescending(x => x.UserID).ToList();
        ...
        public ActionResult blocked()
        {
            if (Session["AdminID"] != null)
            {
                using (db_jugglejoyEntities ctx = new db_jugglejoyEntities())
                {
                    ...
                    ViewBag.HandlerInUse = TempData["HandlerInUse"] == null ? "" : TempData["HandlerInUse"] as string;
                    ViewBag.HandlerNotInUse = TempData["HandlerNotInUse"] == null ? "" : TempData["HandlerNotInUse"] as string;
                    var bloked_Assistants = ctx.tbl_user.Where(x => x.Status == (int)UserStatus.Blocked && x.UserType == "H").OrderByDescending(x => x.UserID).ToList();
                    ...
                    if (IdsToDelete != null && IdsToDelete.Count() > 0)
                    {
                        ...
                        if (action == "Delete")
                        {
                            ...
                            if(user.status == (int)UserStatus.Active)
                            {
                                ...
                                {
                                    var assigned_assistant = ctx.tbl_user.Where(x => x.HandlerID == item.UserID).FirstOrDefault();
                                    if (assigned_assistant == null)
                                    {
                                        handlernotUse = handlernotUse + item.FirstName + ",";
                                        ctx.tbl_user.Remove(item);
                                        ctx.SaveChanges();
                                    }
                                    else
                                    {
                                        handlerinUse = handlerinUse + item.FirstName + ",";
                                    }

                                }
                                if (handlerinUse != "")
                                {
                                    handlerinUse = handlerinUse.TrimEnd(',');
                                    TempData["HandlerInUse"] = "Handlers (" + handlerinUse + ") is/are assigned Assistants and can't be deleted";
                                }
                                if (handlernotUse != "")
                                {
                                    handlernotUse = handlernotUse.TrimEnd(',');
                                    TempData["HandlerNotInUse"] = "Handlers (" + handlernotUse + ") is/are deleted";
                                }
                                return RedirectToAction("active");
                            }
                            else
                        ...
                        else if (action == "Un-Block")
                        {
                            ...
                            if (ctx.SaveChanges() > 0)
                            {                              
                                TempData["ActiveAssistantListMsg"] = "Handler account un-blocked successfully";
                                return RedirectToAction("active");
                            }
                            else
                            {
                                TempData["BlockedAssistantListMsg"] = "Failed to un-block handler account";
                                return RedirectToAction("blocked");
                            }
                        }
                        else if (action == "Block")
                        {
                            ...
                            {
                                handlerinUse = handlerinUse.TrimEnd(',');
                                TempData["HandlerInUse"] = "Handlers (" + handlerinUse + ") can't be blocked,assigned with assistants";
                                return RedirectToAction("active");
                            }
                            else
                            {
                                ...
                                if (handlernotUse != "")
                                {
                                    TempData["HandlerNotInUse"] = "Handlers (" + handlernotUse + ") is/are blocked";
                                }
